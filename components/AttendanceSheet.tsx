...
const AttendanceSheet: React.FC<AttendanceSheetProps> = ({ students, classes, subjects, existingHistory, lang, userRole, onSaveAttendance, onDeleteSession, onDeleteRecord }) => {
  ...
  // Teacher Mode: Time Slot
  const [startTime, setStartTime] = useState<string>('08:00');
  const [endTime, setEndTime] = useState<string>('09:00');

  // Admin Mode: Duration (Hours)
  const [durationHours, setDurationHours] = useState<number>(1);

  // Calculate duration in minutes
  const sessionDuration = useMemo(() => {
    // If Admin, use the manual duration input (converted to minutes)
    if (userRole === 'admin') {
      return Math.round(durationHours * 60);
    }

    // If Teacher, calculate from start/end time
  ...
  }, [startTime, endTime, durationHours, userRole]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div>
            <label className="block text-sm font-bold text-slate-800 mb-2">
              {userRole === 'admin' ? t('durationHours') : t('timeSlot')}
            </label>
              ...
              <div className="relative">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (val >= 0) setDurationHours(val);
                  }}
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary outline-none text-slate-900 font-bold shadow-sm"
                />
                <span className="absolute top-3.5 right-10 text-slate-400 text-sm font-bold rtl:right-auto rtl:left-10 pointer-events-none">
                  {t('hours')}
                </span>
              </div>
            ) : (
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto justify-center">
              ...
              <div className="flex gap-2 w-full sm:w-auto">
                <button
                  ...
                  className={`p-3 md:p-2.5 rounded-lg border transition-all font-bold flex-1 sm:flex-none flex justify-center ${isEditing
                    ...
                    <tr key={student.id} className={`transition-colors duration-150 ${currentStatus === AttendanceStatus.ABSENT ? 'bg-red-50' : 'bg-white hover:bg-slate-50'}`}>
                      ...
                      <td className={`p-4 md:p-5 ${dir === 'rtl' ? 'text-right' : 'text-left'}`}>
                        <div className="flex items-center gap-3 md:gap-4">
                          <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center text-base md:text-lg font-bold shadow-sm border-2 ${student.riskScore > 50 ? 'bg-red-50 text-red-700 border-red-200' : 'bg-slate-50 border-slate-200 text-slate-600'
                            }`}>
                            ...
                            {[
                              ...
                              return (
                                <button
                                  ...
                                  className={`flex-1 flex flex-col items-center justify-center gap-1 py-1.5 md:py-2 px-1 md:px-2 rounded-md text-[10px] md:text-sm font-bold transition-all duration-200 ${isActive
