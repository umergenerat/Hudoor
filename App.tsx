...
const App: React.FC = () => {
  ...
  // Save or Update Attendance (Supabase)
  const handleSaveAttendance = async (newRecords: AttendanceRecord[]) => {
    try {
      await dataService.saveAttendanceRecords(newRecords);
      // Refresh students to update their stats (absence count etc)
      const updatedStudents = await dataService.getStudents();
      setStudents(updatedStudents);

      // Update local history for current view stability
      setAttendanceHistory(prev => {
        const remainingRecords = prev.filter(p =>
          !newRecords.some(n => n.studentId === p.studentId && n.date === p.date)
        );
        return [...remainingRecords, ...newRecords];
      });
      alert(TRANSLATIONS.recordsSaved[lang]);
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-teal-200 font-bold">Loading Hudoor AI...</p>
        </div>
      </div>
    );
  }
